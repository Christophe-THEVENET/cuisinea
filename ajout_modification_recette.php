...
// ------------------------------------ soumission du formulaire (ajout BDD)
if (isset($_POST['saveRecipe'])) {
  ...
  if (isset($_FILES['file']['tmp_name']) && $_FILES['file']['tmp_name'] != '') {
    ...
    if ($checkImage !== false) {
      // Si c'est une image on traite (uniqid méthode PHP retourne une chaine unique, slugify )
  ...
  // pour sauvegarder les valeurs qu'on a entré ds les input en cas d'erreur
  $recipe = [
    'title' => $_POST['title'],
  ...
  <div class="mb-3">
    ...
    <select name="category" id="category" class="form-select">
      ...
      <?php foreach ($categories as $category) { ?>
        ...
        test pour conserver la catagorie sélectionné après envoi  */ if ($recipe['category_id'] == $category['id']) {
